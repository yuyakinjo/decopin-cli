...
export default function createEnv(
  context: EnvContext<typeof process.env>
): EnvHandler {
  // Can access existing environment during definition
  const defaultPort = context.env.DEFAULT_PORT
    ? parseInt(context.env.DEFAULT_PORT)
    : 3000;
  ...
  return {
    ...
    AUTH_TOKEN: {
      ...
      minLength: 32,
      errorMessage:
        'AUTH_TOKEN is required in production and must be at least 32 characters',
    },
    SUPPORT_EMAIL: {
      type: SCHEMA_TYPE.STRING,
      required: false,
