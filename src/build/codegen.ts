...
/** JSON を生成コードへ埋め込んでも、script 要素や行を抜けられない形にする */
function stringifyForCode(value: unknown): string {
  const json = JSON.stringify(value);
  if (json === undefined) {
    throw new TypeError('Cannot serialize value into generated code');
  }
...
function importer(outDir: string, file: string): string {
  return `() => import(${stringifyForCode(toSpecifier(outDir, file))})`;
}
...
export function generateRoutes(input: RoutesInput, outDir: string): string {
  const entries = input.routes.map((route) => {
    ...
    if (verdicts !== undefined) {
      loaders.push(`    effects: ${stringifyForCode(verdicts)},`);
    }

    return `  ${stringifyForCode(route.name)}: {\n${loaders.join('\n')}\n  },`;
  });
  ...
  const helps = [...(input.helpFiles ?? new Map())]
    ...
    .map(
      ([dir, file]) => `  ${stringifyForCode(dir)}: ${importer(outDir, file)},`
    )
    .join('\n');
...
run(routes, {
  program: ${stringifyForCode(program)},
  bin: ${stringifyForCode(bin)},
  globalError,
  notFound,
  helps,
  envFile,
