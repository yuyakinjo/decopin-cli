...
import { hasVersionFile, parseVersionFile, getVersionInfo } from './version-parser.js'
...
export const other = "something"
  ...
  it('should warn when version file not found', async () => {
    ...
    expect(result.warnings).toContain('version.ts file not found in app directory')
    expect(result.versionInfo).toBeNull()
  })
