...
describe('HandlerExecutor', () => {
  ...
  describe('getExecutionOrder', () => {
    test('should return all handlers sorted by execution order', () => {
      ...
      for (let i = 1; i < handlers.length; i++) {
        expect(handlers[i].executionOrder).toBeGreaterThanOrEqual(
          handlers[i - 1].executionOrder
        );
      }
    });
  ...
  describe('validateDependencies', () => {
    test('should validate when all dependencies are satisfied', () => {
      const executor = new HandlerExecutor();
      const result = executor.validateDependencies([
        'env',
        'params',
        'command',
      ]);
    ...
    test('should report missing dependencies', () => {
      ...
      expect(result.errors).toContain(
        "Handler 'params' depends on 'env' which is not available"
      );
    });
  ...
  describe('buildContext', () => {
    test('should build context with env handler', async () => {
      ...
      const envHandler = async (_ctx: Context<Record<string, unknown>>) => ({
        NODE_ENV: 'test',
      });
      ...
      const result = await executor.buildContext(
        initialContext,
        loadedHandlers
      );
    ...
    test('should build context with multiple handlers', async () => {
      ...
      const versionHandler = async (
        _ctx: Context<Record<string, unknown>>
      ) => ({
        version: '1.0.0',
      });
      const paramsHandler = async (_ctx: Context<Record<string, unknown>>) => ({
        schema: {},
        mappings: [],
      });
    ...
    test('should handle middleware and global-error handlers', async () => {
      ...
      const globalErrorHandler = async (_error: unknown) => {};
    ...
    test('should throw error if handler execution fails', async () => {
      ...
      await expect(
        executor.buildContext(initialContext, loadedHandlers)
      ).rejects.toThrow("Failed to execute handler 'env': Handler failed");
    });
  });
